...
const ADV_ID = '7468288053866561553'; // AI_1

const ADS = [
  { cr: 'CR01199', adId: '1861923903237570', lp: 'LP1' },
  { cr: 'CR01200', adId: '1861923921044514', lp: 'LP1' },
  { cr: 'CR01201', adId: '1861923923656722', lp: 'LP1' },
  { cr: 'CR01202', adId: '1861923929431281', lp: 'LP1' },
  { cr: 'CR01203', adId: '1861923947011265', lp: 'LP1' },
];

// 出稿日: 2026-04-09, 昨日: 2026-04-11
...
async function countInSheet(
  ...
  try {
    ...
    if (pathCol === -1 || dateCol === -1) { console.log(`  [warn] ${sheetName}: no path/date col`); return 0; }
    let count = 0;
...
async function getSmartPlusMetrics(advertiserId: string, startDate: string, endDate: string) {
  ...
  while (true) {
    ...
    if (d.code !== 0) { console.log(`  [err SP metrics] ${d.message}`); break; }
...
async function main() {
  ...
  for (const a of ADS) {
    ...
    const regPath = `TikTok広告-AI-${a.lp}-${a.cr}`;
    ...
    const label = `AI全部やめました渋谷Ver/${a.lp}-${a.cr}`;
    console.log(`${label}: spend=¥${spend}, imp=${imp}, clicks=${m.clicks}, CV=${cv}, front=${front} (OTO:${frontOTO}+3day:${front3day})`);

    rows.push([label, spend, imp, cpm, ctr, cpa, front, cpo].join(','));
  }

  const outPath = path.join(__dirname, 'exports', 'yamemashita_5crs_result.csv');
