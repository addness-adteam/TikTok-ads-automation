...
} from './domain/profit-simulation';
import {
  judgeDirection,
  calculateRequiredAcquisition,
} from './domain/direction-judgment';
...
export class ProfitSimulationService {
  ...
  constructor(
    ...
    @Inject(WINNING_CREATIVE_SOURCE)
    private readonly winningCreativeSource: WinningCreativeSource,
  ...
  async run(channelFilter?: ChannelType): Promise<{
    ...
    for (const channelType of channels) {
      try {
        ...
        const metrics = await this.metricsDataSource.getMonthlyMetrics(
          channelType,
          year,
          month,
        );
        const kpi = await this.metricsDataSource.getKPI(
          channelType,
          year,
          month,
        );
        const targetProfit = await this.metricsDataSource.getTargetProfit(
          channelType,
          year,
          month,
        );
        ...
        const actualDays =
          metrics.dailyData.filter((d) => d.adSpend > 0 || d.optins > 0)
            .length || dayOfMonth;
        ...
        const cpa =
          metrics.optinCount > 0 ? metrics.adSpend / metrics.optinCount : 0;
        ...
        const targetROAS =
          metrics.adSpend > 0
            ? (targetProfit + simulation.projectedAdSpend) /
              simulation.projectedAdSpend
            : kpi.targetROAS;
        const currentROAS =
          metrics.adSpend > 0 ? metrics.totalRevenue / metrics.adSpend : 0;
        ...
        this.logger.log(
          `[${channelType}] 判定結果: ${judgment.direction} - ${judgment.reason}`,
        );
    ...
    // 全導線サマリー
    const summary = calculateTotalProfitSummary(channelResults, {
      year,
      month,
    });
    ...
    this.logger.log(
      `シミュレーション完了: 粗利推定 ${summary.totalProjectedProfit.toLocaleString()}円`,
    );
