...
import { AllowableSeminarSeatCpo, YearMonth } from '../value-objects/allowable-seminar-seat-cpo';
...
const makeAd = (override: Partial<Parameters<typeof AdUnderEvaluation.create>[0]>) =>
...
describe('AlertRuleEvaluator', () => {
  ...
  describe('配信期間', () => {
    it('4日経過はアラート対象外', () => {
      const ad = makeAd({
        deliveryPeriod: DeliveryPeriod.between(jst(2026, 4, 1), jst(2026, 4, 5)),
      });
  ...
  describe('着座0件', () => {
    it('spend >= 許容CPO → アラート (reason=ZERO_ATTENDANCE_WITH_SPEND)', () => {
      const ad = makeAd({ totalSpend: JPY.of(15000), attendanceCount: 0, reservationCount: 5 });
      const decision = evaluator.evaluate(ad, allowable, false);
      expect(decision.shouldAlert).toBe(true);
      expect(decision.reason).toBe('ZERO_ATTENDANCE_WITH_SPEND');
    });
    it('spend > 許容CPO → アラート', () => {
      const ad = makeAd({ totalSpend: JPY.of(50000), attendanceCount: 0, reservationCount: 0 });
      const decision = evaluator.evaluate(ad, allowable, false);
      expect(decision.shouldAlert).toBe(true);
    });
    it('spend < 許容CPO → アラート対象外 (まだ予算消化不十分)', () => {
      const ad = makeAd({ totalSpend: JPY.of(14999), attendanceCount: 0, reservationCount: 0 });
      const decision = evaluator.evaluate(ad, allowable, false);
      expect(decision.shouldAlert).toBe(false);
    });
  });
  ...
  describe('detail出力', () => {
    ...
    it('着座0件時のoverageRateはnull', () => {
      const ad = makeAd({ totalSpend: JPY.of(20000), attendanceCount: 0, reservationCount: 0 });
      const decision = evaluator.evaluate(ad, allowable, false);
