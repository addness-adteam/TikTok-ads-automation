...
async function testBillingEvents() {
  console.log('🎯 billing_event 値テスト開始\n');
  ...
  // テストする billing_event の候補
  const billingEvents = [
    'OCPM',
    'CPM',
    'CPC',
    'CPV',
    'INVALID_VALUE', // エラーメッセージで有効な値を確認
  ];

  for (const billingEvent of billingEvents) {
    console.log(`\n${'='.repeat(80)}`);
    console.log(`テスト: billing_event = "${billingEvent}"`);
    console.log('='.repeat(80));

    try {
      // Campaign作成
      const timestamp = new Date().toISOString().slice(0, 19).replace(/[T:-]/g, '');
      const campaignName = `テスト_${billingEvent}_${timestamp}`;
      ...
      const adgroupName = `${dateStr} ${billingEvent}`;
      ...
      if (adgroupResponse.data.code === 0) {
        console.log(`✅ ✅ ✅ AdGroup作成成功！ billing_event="${billingEvent}" が有効です！`);
        console.log(`   AdGroup ID: ${adgroupResponse.data.data.adgroup_id}`);
        console.log('\n🎉 正しい billing_event を見つけました！');
        break;
      } else {
      ...
      if (error.response?.data) {
        ...
        // billing_eventに関する具体的なエラーなら表示
        if (errorData.message.includes('billing_event')) {
          console.log('   ⚠️  billing_eventに関連するエラーです');
          console.log('   📋 完全なエラーメッセージ:', JSON.stringify(errorData, null, 2));
        }
      } else {
...
testBillingEvents().catch(console.error);
