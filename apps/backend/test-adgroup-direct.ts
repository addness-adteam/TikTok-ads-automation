...
async function testAdGroupDirect() {
  console.log('🎯 AdGroup作成直接テスト開始\n');
  ...
  const campaignName = 'テストキャンペーン_AdGroup_' + timestamp;
  ...
  try {
    ...
    console.log(`   Campaign ID: ${campaignResponse.data.data.campaign_id}\n`);

    const campaignId = String(campaignResponse.data.data.campaign_id);

    // AdGroup作成リクエストボディを構築
    console.log('📦 ステップ2: AdGroup作成リクエスト準備中...\n');

    const today = new Date();
    const dateStr = `${String(today.getFullYear()).slice(2)}${String(today.getMonth() + 1).padStart(2, '0')}${String(today.getDate()).padStart(2, '0')}`;
    const adgroupName = `${dateStr} テストノンタゲ`;
    ...
    const requestBody = {
      ...
      conversion_id: '7388093644378554384', // ✅ Changed from optimization_event to conversion_id
    ...
    // AdGroup作成リクエスト送信
    console.log('🚀 ステップ3: AdGroup作成APIにリクエスト送信中...\n');
    ...
    console.log('\n' + '═'.repeat(80));
    console.log('🎉 テスト成功: AdGroup作成が正常に完了しました！');
    console.log('═'.repeat(80));
...
testAdGroupDirect().catch(console.error);
