...
async function testLeadOptimization() {
  console.log('🎯 LEAD系 optimization_goal テスト開始\n');
  ...
  // LEAD_GENERATION キャンペーン用のoptimization_goal候補
  const leadOptimizationGoals = [
    'LEADS',
    'CONVERSION_LEADS',
    'PREFERRED_LEAD',
    'LEAD_WEB_ENGAGEMENT',
  ...
  for (const optimizationGoal of leadOptimizationGoals) {
    console.log(`\n${'='.repeat(80)}`);
    console.log(`テスト: optimization_goal = "${optimizationGoal}" with BID_TYPE_MAX_CONVERSION`);
    ...
    try {
      ...
      if (adgroupResponse.data.code === 0) {
        ...
        console.log(`   optimization_goal="${optimizationGoal}" が有効です！`);
...
testLeadOptimization().catch(console.error);
