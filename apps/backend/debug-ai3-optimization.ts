...
async function bootstrap() {
  ...
  try {
    ...
    if (advertisers.length === 0) {
      console.log('❌ AI3という名前のAdvertiserが見つかりませんでした');
      console.log('\n全Advertiserを表示:');
      ...
      allAdvertisers.forEach((adv, i) => {
        console.log(`  [${i + 1}] ${adv.name} (ID: ${adv.tiktokAdvertiserId})`);
      });
      await app.close();
      return;
    }
    ...
    for (const campaign of activeCampaigns.slice(0, 5)) {
      ...
      console.log(`  全てCR名: ${allHaveCreativeNames ? 'Yes (Phase 2対象)' : 'No'}`);
      console.log(`  手動広告名あり: ${hasManualNames ? 'Yes (Phase 1で処理済み)' : 'No'}`);
      console.log('');
    }
