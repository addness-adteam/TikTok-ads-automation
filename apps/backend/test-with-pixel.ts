...
async function testWithPixel() {
  console.log('🎯 Pixel + Conversion ID付きテスト開始\n');
  ...
  try {
    ...
    const campaignName = 'テスト_WithPixel_' + timestamp;
    ...
    // AdGroup作成 - Pixel + Conversion ID 付き
    console.log('📦 ステップ2: AdGroup作成中 (Pixel + Conversion ID付き)...\n');

    const today = new Date();
    const dateStr = `${String(today.getFullYear()).slice(2)}${String(today.getMonth() + 1).padStart(2, '0')}${String(today.getDate()).padStart(2, '0')}`;
    const adgroupName = `${dateStr} WithPixel`;
    ...
    const requestBody = {
      ...
      pixel_id: '7388088697557663760', // SNS1 2 専用 Pixel ID
      conversion_id: '7388093644378554384', // Event ID: ON_WEB_REGISTER
    ...
    if (adgroupResponse.data.code === 0 && adgroupResponse.data.data?.adgroup_id) {
      ...
      console.log('✅ pixel_id と conversion_id が必要でした！');
...
testWithPixel().catch(console.error);
