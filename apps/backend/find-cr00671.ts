...
async function scan(endpoint: string, acc: {name:string, id:string}) {
  ...
  while (true) {
    ...
    for (const ad of list) {
      if ((ad.ad_name || '').includes('CR00671') && (ad.ad_name || '').includes('箕輪')) {
        found.push({ acc: acc.name, accId: acc.id, type: isSP?'SP':'REG', ad_id: ad.ad_id || ad.smart_plus_ad_id, ad_name: ad.ad_name, status: ad.operation_status });
      }
    }
...
async function main() {
  for (const acc of ACCOUNTS) {
    ...
    for (const ad of [...reg, ...sp]) {
      console.log(`${ad.acc} (${ad.accId}) | ${ad.type} | ${ad.ad_id} | ${ad.status} | ${ad.ad_name}`);
    }
  }
}
main();
