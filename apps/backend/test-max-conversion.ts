...
async function testMaxConversion() {
  console.log('🎯 BID_TYPE_MAX_CONVERSION テスト開始\n');
  ...
  try {
    ...
    const campaignName = 'テスト_MaxConversion_' + timestamp;
    ...
    // AdGroup作成 - BID_TYPE_MAX_CONVERSION使用
    console.log('📦 ステップ2: AdGroup作成中 (BID_TYPE_MAX_CONVERSION)...\n');

    const today = new Date();
    const dateStr = `${String(today.getFullYear()).slice(2)}${String(today.getMonth() + 1).padStart(2, '0')}${String(today.getDate()).padStart(2, '0')}`;
    const adgroupName = `${dateStr} MaxConversion`;
    ...
    if (adgroupResponse.data.code === 0 && adgroupResponse.data.data?.adgroup_id) {
      ...
      console.log('✅ bid_type: BID_TYPE_MAX_CONVERSION が正しい設定です！');
...
testMaxConversion().catch(console.error);
