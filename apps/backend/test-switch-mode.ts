...
async function main() {
  ...
  const camp = await api('/v1.3/smart_plus/campaign/create/', {
    advertiser_id: ADV, campaign_name: 'TEST_SWITCH_MODE_DELETE',
  ...
  const ag = await api('/v1.3/smart_plus/adgroup/create/', {
    advertiser_id: ADV, campaign_id: campId, adgroup_name: 'TEST_SWITCH',
    ...
    schedule_type: 'SCHEDULE_FROM_NOW', schedule_start_time: '2026-04-09 15:00:00',
  ...
  // 通常APIでBUDGET_MODE_DAYに変更を試みる
  console.log('\n通常APIでBUDGET_MODE_DAYに変更...');
  const update1 = await api('/v1.3/adgroup/update/', {
    advertiser_id: ADV, adgroup_id: agId, budget_mode: 'BUDGET_MODE_DAY', budget: 3000,
  });
  console.log('結果:', update1.code, update1.message);

  if (update1.code === 0) {
    const check = await get('/v1.3/adgroup/get/', { advertiser_id: ADV, filtering: JSON.stringify({ adgroup_ids: [agId] }), fields: JSON.stringify(['adgroup_id', 'budget', 'budget_mode']) });
    console.log('通常API:', check.data?.list?.[0]?.budget, check.data?.list?.[0]?.budget_mode);
    const spCheck = await get('/v1.3/smart_plus/adgroup/get/', { advertiser_id: ADV, adgroup_ids: JSON.stringify([agId]) });
    console.log('SP API:', spCheck.data?.list?.[0]?.budget, spCheck.data?.list?.[0]?.budget_mode);
  }
