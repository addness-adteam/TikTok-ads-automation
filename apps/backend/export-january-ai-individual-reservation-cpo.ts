/**
 * AI導線 1月の個別予約CRごとの個別予約CPO算出スクリプト
 *
 * 対象期間: 2026/1/1 〜 2026/1/31
 * 対象: 個別予約があったCRのみ
...
async function main() {
  try {
    console.log('=== AI導線 1月 CRごとの個別予約CPO算出 ===\n');
    ...
    if (noSpendCRs.length > 0) {
      ...
      console.log('  → 1月に配信されていないCR、または広告名のLP-CRパターンが一致しない可能性があります');
    }
    ...
    const bom = '\uFEFF';
    const csvContent = [
      `AI導線 1月 個別予約CPO一覧（${START_DATE} 〜 ${END_DATE}）`,
      '対象: 個別予約があったCRのみ',
      '',
      csvHeaders.join(','),
      ...csvRows,
    ...
    const outputPath = path.join(outputDir, `AI導線_1月_個別予約CPO_${START_DATE}_${END_DATE}.csv`);
