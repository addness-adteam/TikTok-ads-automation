...
async function testTrafficProperBidding() {
  console.log('🎯 TRAFFIC objective (正しい入札設定) テスト開始\n');
  ...
  try {
    ...
    const campaignName = 'テスト_TRAFFIC_Bidding_' + timestamp;
    ...
    // AdGroup作成 - BID_TYPE_CUSTOM with bid_price
    console.log('📦 ステップ2: AdGroup作成中 (BID_TYPE_CUSTOM)...\n');

    const today = new Date();
    const dateStr = `${String(today.getFullYear()).slice(2)}${String(today.getMonth() + 1).padStart(2, '0')}${String(today.getDate()).padStart(2, '0')}`;
    const adgroupName = `${dateStr} TRAFFIC Custom`;
    ...
    if (adgroupResponse.data.code === 0 && adgroupResponse.data.data?.adgroup_id) {
      ...
      console.log('✅ TRAFFICでは AdGroup が作成できました');
      console.log('⚠️  LEAD_GENERATIONに問題があると特定できます');
...
testTrafficProperBidding().catch(console.error);
