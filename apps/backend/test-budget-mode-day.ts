...
async function main() {
  ...
  const camp = await api('/v1.3/smart_plus/campaign/create/', {
    advertiser_id: ADV, campaign_name: 'TEST_BUDGET_MODE_DAY_DELETE',
  ...
  const ag = await api('/v1.3/smart_plus/adgroup/create/', {
    advertiser_id: ADV, campaign_id: campId, adgroup_name: 'TEST_MODE_DAY',
    budget_mode: 'BUDGET_MODE_DAY', budget: 3000,
  ...
  if (ag.code !== 0) {
    console.log('エラー:', ag.code, ag.message);
  } else {
    const agId = String(ag.data.adgroup_id);
    console.log('agId:', agId);

    // 両APIで確認
    const sp = await get('/v1.3/smart_plus/adgroup/get/', { advertiser_id: ADV, adgroup_ids: JSON.stringify([agId]) });
    const normal = await get('/v1.3/adgroup/get/', { advertiser_id: ADV, filtering: JSON.stringify({ adgroup_ids: [agId] }), fields: JSON.stringify(['adgroup_id', 'budget', 'budget_mode']) });
    
    console.log('SP API:', 'budget=' + sp.data?.list?.[0]?.budget, 'mode=' + sp.data?.list?.[0]?.budget_mode);
    console.log('通常API:', 'budget=' + normal.data?.list?.[0]?.budget, 'mode=' + normal.data?.list?.[0]?.budget_mode);
  }
