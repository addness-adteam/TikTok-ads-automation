...
async function testCreativeUpload() {
  console.log('🎬 Creative アップロードテスト開始\n');

  const tiktokAdvertiserId = '7247073333517238273';
  const advertiserId = 'ce834117-1819-4a6e-9451-89fbb49e326f'; // UUID
  const videoPath = 'C:\\Users\\itali\\Downloads\\1年後悔_冒頭4_AI.mp4';
  ...
  console.log(`✅ ファイル確認成功: ${path.basename(videoPath)} (${fileSizeMB} MB)\n`);
  ...
  formData.append('name', 'テスト動画_' + new Date().toISOString());
  ...
  try {
    ...
    if (creative.tiktokVideoId) {
      ...
      console.log('❌ TikTok Video ID が見つかりません');
    }
    ...
    if (dbCreative) {
      ...
      console.log('   File Size: ' + (dbCreative.fileSize ? (dbCreative.fileSize / (1024 * 1024)).toFixed(2) + ' MB' : 'N/A'));
    } else {
      console.log('❌ DBに保存されていません');
    }

    console.log('\n' + '═'.repeat(60));
    console.log('🎉 テスト完了: 動画アップロード機能は正常に動作しています！');
    console.log('═'.repeat(60));
...
testCreativeUpload().catch(console.error);
