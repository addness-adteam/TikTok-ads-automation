 ...
 *
 * 対象期間: 2026/2/1 〜 2026/2/13 に個別予約があったCR
 * 出力: CSV（広告名、消化金額、個別予約CPO）
 */
...
async function main() {
  try {
    ...
    const bom = '\uFEFF';
    const csvContent = [
      `AI導線 個別予約CPO一覧（${START_DATE} 〜 ${END_DATE}）`,
      '',
      csvHeaders.join(','),
      ...csvRows,
    ].join('\n');

    const outputPath = path.join(outputDir, `AI導線_個別予約CPO_${START_DATE}_${END_DATE}.csv`);
