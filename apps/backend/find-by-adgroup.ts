...
async function scan(endpoint: string, name: string, id: string) {
  ...
  while (true) {
    const qs = new URLSearchParams({
      advertiser_id: id,
      fields: JSON.stringify(isSP ? ['smart_plus_ad_id','ad_name','operation_status','adgroup_id'] : ['ad_id','ad_name','operation_status','adgroup_id']),
