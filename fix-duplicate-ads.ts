...
async function main() {
  ...
  for (const pair of duplicatePairs) {
    ...
    // ad_id形式のレコードに紐付いているメトリクスをsmart_plus_ad_id形式に移行
    if (adByAdId._count.metrics > 0) {
      await prisma.metric.updateMany({
        where: { adId: adByAdId.id },
        data: { adId: adBySmartPlusId.id }
      });
      console.log(`  メトリクス ${adByAdId._count.metrics}件を移行`);
    }

    // AdPerformanceも移行（存在する場合）
    const adPerformance = await prisma.adPerformance.findUnique({
      where: { adId: adByAdId.id }
    });
    if (adPerformance) {
      // 既存のAdPerformanceがある場合は削除してから移行
      const existingPerformance = await prisma.adPerformance.findUnique({
      ...
      if (!existingPerformance) {
        ...
        console.log(`  AdPerformanceを移行`);
      } else {
        await prisma.adPerformance.delete({
          where: { adId: adByAdId.id }
        });
        console.log(`  古いAdPerformanceを削除（新しい方を保持）`);
      }
    }
